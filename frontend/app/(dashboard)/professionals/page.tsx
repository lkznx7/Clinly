    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-white rounded-lg border border-gray-200 p-3">
          ...
          <p className="text-xl font-bold text-gray-900 mt-0.5">{totalStats.total}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-3">
          <p className="text-xs text-gray-500">Ativos</p>
          <p className="text-xl font-bold text-green-600 mt-0.5">{totalStats.active}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-3">
          <p className="text-xs text-gray-500">Total Pacientes</p>
          <p className="text-xl font-bold text-gray-900 mt-0.5">{totalStats.totalPatients}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-3">
          <p className="text-xs text-gray-500">Avaliação Média</p>
          <p className="text-xl font-bold text-yellow-600 mt-0.5">{totalStats.avgRating}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map((pro) => (
          <div key={pro.id} className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-sm transition-shadow">
            <div className="flex items-start gap-3">
              <div className="h-10 w-10 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-medium">
                {pro.initials}
              </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              <div className="text-center">
                <p className="text-sm font-bold text-gray-900">{pro.patients}</p>
                <p className="text-xs text-gray-500">Pacientes</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-bold text-gray-900">{pro.appointments}</p>
                <p className="text-xs text-gray-500">Consultas/Sem</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-bold text-gray-900">{pro.since.split("-")[0]}</p>
            ...
            {pro.specializations && (
              <div className="mt-2.5 flex flex-wrap gap-1">
                {pro.specializations.map((spec) => (
                  <span key={spec} className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600">
                    {spec}
                  </span>
                ))}
              </div>
      ...
      {filtered.length === 0 && (
        <div className="bg-white rounded-lg border border-gray-200 px-4 py-8 text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">Nenhum profissional encontrado</p>
        </div>
      )}
    </div>
  );
